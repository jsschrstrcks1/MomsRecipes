...
# Soli Deo Gloria.
# Leaf adoption: configuration and execute bits are observable here; the full
# canonical guard chain named in the historical notes is NOT installed here.
# Do not turn successful configuration into a claim about downstream checks.
...
GIT_INVOKED_HOOKS="pre-commit pre-push prepare-commit-msg commit-msg pre-merge-commit post-merge"
not_runnable=""
...
if [ -n "$current" ]; then
  if [ "$current" != ".githooks" ]; then
    echo "[arm-hooks-path] Preserving configured core.hooksPath=$current; custom hook execution was not evaluated."
    exit 0
  fi
  ...
  if [ -n "$not_runnable" ]; then
    ...
    echo "[arm-hooks-path] These installed hook entry points cannot execute; downstream guards were not verified." \
...
if git -C "$root" config core.hooksPath .githooks 2>/dev/null; then
  ...
  if [ -n "$not_runnable" ]; then
    ...
    echo "[arm-hooks-path] WARNING: these hooks are NOT executable and git will SKIP them:" \
         "$not_runnable" >&2
    echo "[arm-hooks-path] core.hooksPath is now set, but these entry points cannot execute." \
         "Run: chmod +x $root/.githooks/*" >&2
  else
    echo "[arm-hooks-path] Installed git hook entry points are configured and executable; downstream guard behavior was not verified."
  fi
else
  ...
  echo "[arm-hooks-path] Repository-provided hooks were not activated. Check repository permissions and configure core.hooksPath explicitly." >&2
fi
